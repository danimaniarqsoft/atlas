'use strict';

describe('Controller Tests', function() {

    describe('Norma Management Detail Controller', function() {
        var $scope, $rootScope;
        var MockEntity, MockPreviousState, MockNorma, MockEstatusCat, MockPaisCat, MockTipoNormaCat;
        var createController;

        beforeEach(inject(function($injector) {
            $rootScope = $injector.get('$rootScope');
            $scope = $rootScope.$new();
            MockEntity = jasmine.createSpy('MockEntity');
            MockPreviousState = jasmine.createSpy('MockPreviousState');
            MockNorma = jasmine.createSpy('MockNorma');
            MockEstatusCat = jasmine.createSpy('MockEstatusCat');
            MockPaisCat = jasmine.createSpy('MockPaisCat');
            MockTipoNormaCat = jasmine.createSpy('MockTipoNormaCat');
            

            var locals = {
                '$scope': $scope,
                '$rootScope': $rootScope,
                'entity': MockEntity,
                'previousState': MockPreviousState,
                'Norma': MockNorma,
                'EstatusCat': MockEstatusCat,
                'PaisCat': MockPaisCat,
                'TipoNormaCat': MockTipoNormaCat
            };
            createController = function() {
                $injector.get('$controller')("NormaDetailController", locals);
            };
        }));


        describe('Root Scope Listening', function() {
            it('Unregisters root scope listener upon scope destruction', function() {
                var eventType = 'atlasApp:normaUpdate';

                createController();
                expect($rootScope.$$listenerCount[eventType]).toEqual(1);

                $scope.$destroy();
                expect($rootScope.$$listenerCount[eventType]).toBeUndefined();
            });
        });
    });

});
